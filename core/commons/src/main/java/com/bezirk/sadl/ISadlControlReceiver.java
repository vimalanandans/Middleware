...
public interface ISadlControlReceiver {
     ...
     * @param serviceId   - UhuZirkId of the Service
     * @return true if registered, false otherwise.
     */
    public Boolean isStreamTopicRegistered(final String streamTopic, final UhuZirkId serviceId);

    /**
     * Returns all {@link UhuDiscoveredZirk} that are subscribed to pRole.
     *
     ...
     */
    public Set<UhuDiscoveredZirk> discoverServices(final ProtocolRole pRole, final Location location);

    /**
     * Returns the Location of the Service.
     *
     * @param serviceId UhuZirkId of the Service whose location needs to be known
     * @return Location if exists, null if the service is not registered
     */
    public Location getLocationForService(final UhuZirkId serviceId);
