 ...
 */
public class FileCreationHelper implements  RemoteLoggingMessageNotification {
    ...
    @Override
    public void handleLogMessage(RemoteLoggingMessage remoteLogMessage)throws IOException{
        logger.debug("create file with timestamp and store the remotelogging message");
        Date date = new Date() ;
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH-mm-ss") ;
        logger.debug("dateformat is "+dateFormat.format(date));
        File file = new File("C:\\Users\\psg6kor\\Desktop\\"+dateFormat.format(date) + ".txt") ;
        BufferedWriter out = new BufferedWriter(new FileWriter(file));
        logger.debug("Sphere Name in file "+remoteLogMessage.sphereName);
        logger.debug("timestamp in file "+sdf.format(Long.valueOf(remoteLogMessage.timeStamp)));
