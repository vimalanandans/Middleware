		...
		try {
			while(isRunning){
				...
				try{
					...
					if(com.bosch.upa.uhu.remotelogging.util.Util.LOGGING_VERSION.equals(logMsg.version)){
						platformSpecificLogger.handleLogMessage(logMsg);
					}else{
						log.error("LOGGING VERSION MISMATCH!!" + "Received LOG MSG VERSION = " +logMsg.version +
							" CURRENT LOGGING VERSION: " + com.bosch.upa.uhu.remotelogging.util.Util.LOGGING_VERSION);
					}
				}catch(Exception e){
